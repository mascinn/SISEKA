...
const DEMO_ACCOUNTS = [
  { label: 'Admin BPH', username: 'bph', pass: 'barengbareng', role: 'admin' },
  { label: 'Mas Budi (Aleaf)', username: 'aleafcom', pass: 'aleafcom03', role: 'tenant' },
  { label: 'Mas Ipul (Bias)', username: 'biascom', pass: 'biascom04', role: 'tenant' },
  { label: 'Bude Eni (Al-Wasii)', username: 'kantinalwasii', pass: 'kantinalwasii01', role: 'tenant' },
];
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center px-4 py-8 relative">
      <div className="w-full max-w-sm mx-auto space-y-6">
        ...
        <div className="text-center space-y-2">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-forest-900 text-white shadow-sm mb-1">
            <Store className="w-7 h-7 text-forest-100" />
          </div>
        ...
        <div className="bg-white rounded-3xl p-7 shadow-xl border border-slate-200 space-y-4">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 px-4 rounded-full bg-forest-900 hover:bg-forest-950 text-white font-bold text-xs shadow-sm flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:opacity-50 mt-2"
            >
                ...
                <button
                  ...
                  className={`p-3 rounded-2xl text-left text-xs font-semibold border transition-all ${
                    username === acc.username
                      ? 'bg-forest-50 text-forest-900 border-forest-200 shadow-xs'
                      : 'bg-white text-slate-700 border-slate-200 hover:border-slate-300'
                  }`}
                >
                  <div className="font-bold text-[11px] truncate">{acc.label}</div>
                  <div className="text-[10px] text-slate-400 truncate">{acc.username}</div>
                </button>
              ))}
            </div>
          </div>
